...
module.exports = new class {
  ...
  rgxFindTable = (prev, rowSources)=> {
    ...
    const firstPipeIdx = fI(rowSources, !0)
    if (firstPipeIdx === -1) return

    rowSources.splice(0, firstPipeIdx)
    let endIdx = fI(rowSources), contGrid
    while (endIdx > -1) {
      const expected = rowSources.splice(0, endIdx)
      contGrid = this.buildContGrid(expected)
      if (!contGrid) {
        const nextPipeIdx = fI(rowSources, !0)
        if (nextPipeIdx === -1) return

        rowSources.splice(0, nextPipeIdx)
        endIdx = fI(rowSources)
        continue
      }
      prev.r = rowSources
      return contGrid
